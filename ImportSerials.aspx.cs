...
{
    public partial class ImportSerials : System.Web.UI.Page
    {
        ...
        private void BindGridView()
        {
            ...
            {
                SqlCommand sqlCommand = new SqlCommand("GetLastWO", connection);
                ...
                if (fuData.PostedFile.FileName != "")
                {
                    try
                    {
                        ...
                        using (OleDbConnection connection = new OleDbConnection(string.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties=Excel 8.0", (object)filename)))
                        {
                            ...
                            connection.Open();
                            new SqlBulkCopy(ConfigurationManager.ConnectionStrings["traceability"].ConnectionString)
                            {
                                DestinationTableName = "SerialConcentrate",
                                ColumnMappings = {
                  {
                    "WorkOrder",
                    "WorkOrder"
